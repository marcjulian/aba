...
export class Seo {
  ...
  applyFromStrategy(config: SeoConfig, fullTitle: string): void {
    ...
    if (merged.description !== undefined) {
      this.setDescription(merged.description);
    }

    if (merged.robots !== undefined) {
      this.setRobots(merged.robots);
    }
    if (merged.ogType !== undefined) {
      this.meta.updateTag({ property: 'og:type', content: merged.ogType });
    }
    if (merged.ogImage !== undefined) {
      const url = this.resolveUrl(merged.ogImage);
